...
static UCHAR ucDelayAudioInitStatus;
static USHRT usTimerAudioInit;
static USHRT usTimerAudioPacket;
static bit bReceivedAudioPacket;
...
//    SC_AUDIO_DAC_DRIV_CTRL = 0xcc; //test by Rex 0x00
    SC_AUDIO_EXT_CAP_CTRL |= BIT4;
    SC_AUDIO_OUT_CTRL = 0x02;
//	  SC_AUDIO_ANALOG_OUT_CTRL = 0xF4;
		
    usTimerAudioInit = GetTimerTick();
    ucDelayAudioInitStatus = 0x00;
    
    SC_PD_PMCLK |= (BIT6|BIT7);
    SC_AUDIO_AUTO_CTRL8 |= BIT1;//Sigma Delta audio clock gated // 0x8ae
    ...
    SC_HDMI_CLK_INT |= BIT0; // Normal HDMI interrupt// 0x608
    SC_CP_PACKET_TYPE = 0x83;//0x02;
    SC_HDMI_PREAMBLE_CYCLE = 0x06;
    ScalerAudioPowerOn();
}

void HDMIDelayAudioInit(void)
{
    switch (ucDelayAudioInitStatus) {
        case 0x00:
            if ((GetTimerTick()-usTimerAudioInit) > 500) {//delay 500ms
                ucDelayAudioInitStatus = 0x01;
                //SC_AUDIO_ANALOG_OUT_CTRL = 0xF4;
                SC_AUDIO_DAC_DRIV_CTRL = 0x00;
                SC_AUDIO_SELECT = 0x00;
                SC_AUDIO_AUTO_CTRL_8B5 = 0x81;
                SC_AUDIO_TEST1 = 0x07;//test by Rex 0x07;
                SC_AUDIO_OUT_CTRL = 0x03;
                SC_AUDIO_ANALOG_OUT_CTRL = 0x74;
                usTimerAudioInit = GetTimerTick();
            }
            break;
        case 0x01:
            if ((GetTimerTick()-usTimerAudioInit) > 40) {//delay 40ms
                ucDelayAudioInitStatus = 0x02;
                SC_AUDIO_ANALOG_OUT_CTRL = 0x7C;//[N3]vcm run
                SC_AUDIO_EXT_CAP_CTRL &= ~BIT4;//BYP off
                usTimerAudioInit = GetTimerTick();
            }
            break;
        default:
...
void HdmiHandler(void)
{
    ...
    if ((ch != DIGITAL_INPUT0) && (ch != DIGITAL_INPUT1) && (ch != DIGITAL_INPUT2) && (ch != DIGITAL_INPUT3)) {
        return;
    }
    
#if ENABLE_SCALER_AUDIO == ON
    if (SC_DISPLAY_MUTE_CTRL == 0x00) {
        ch = SC_HDMI_INT_STATE1[1];
        if ((ch & (BIT6)) != 0x00) {
            SC_HDMI_MUTE |= BIT1; // 0x737
            usAudioMuteDelay = HDMI_AUDIODELAY;
            SC_HDMI_INT_STATE1[1] = ch;
        }
        else {
    ...
    if (ch != 0) {
        ...
        //}
        if ((ch & BIT1) != 0) {
            bReceivedAudioPacket = TRUE;
            usTimerAudioPacket = GetTimerTick();
//            printf("Received audio packet\r\n");
        }
        else {
            if ((GetTimerTick() - usTimerAudioPacket) > 50) {
                bReceivedAudioPacket = FALSE;
                usTimerAudioPacket = GetTimerTick();
            }
        }
        //if ((ch & BIT0) != 0) {
...
{
    USHRT clock;
    UCHAR phyindex = _DPPP_PHY0;
#if IS_NT68870_SERIES
    UCHAR curr_interface;
    curr_interface = GetCurrInputInterface();
    if ((curr_interface == DIGITAL_INPUT2) || (curr_interface == DIGITAL_INPUT3)) {
        phyindex = _DPPP_PHY1;
    }
#endif
    ...
    if ((SC_HDCP_PACKING & 0x07)==0x04) {
        SC_TMDS_LINK_CLK_SEL |= (BIT7|BIT0);// 0x1e7
    }
    else {
        SC_TMDS_LINK_CLK_SEL &= ~(BIT7|BIT0);// 0x1e7
    }
    ...
    switch (SC_HDCP_PACKING & 0x07) {
        case 4://8 bit
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_DIV, 0x00);//Addr = 0x9C24; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_FREQ_HI, 0x00);//Addr = 0x9C25; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_FREQ_MI, 0x00);//Addr = 0x9C26; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_FREQ_LO, 0x00);//Addr = 0x9C27; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_DIV, 0x00);//Addr = 0x9C20; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_HI, 0x00);//Addr = 0x9C21; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_MI, 0x00);//Addr = 0x9C22; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_LO, 0x00);//Addr = 0x9C23; *Addr = 0x00;
        ...
        case 5://10 bit
            ...
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_FREQ_MI, 0x00);//Addr = 0x9C26; *Addr = 0x00;
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_FREQ_LO, 0x00);//Addr = 0x9C27; *Addr = 0x00;
            if (IsTMDSLBR(phyindex) == TRUE) {
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_DIV, 0xC2);//Addr = 0x9C20; *Addr = 0xC2;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_HI, 0x19);//Addr = 0x9C21; *Addr = 0x19;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_MI, 0x00);//Addr = 0x9C22; *Addr = 0x00;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_LO, 0x00);//Addr = 0x9C23; *Addr = 0x00;
            }
            else {
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_DIV, 0xC0);//Addr = 0x9C20; *Addr = 0xC0;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_HI, 0x19);//Addr = 0x9C21; *Addr = 0x19;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_MI, 0x00);//Addr = 0x9C22; *Addr = 0x00;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_LO, 0x00);//Addr = 0x9C23; *Addr = 0x00;
            }
            SC_DVI_PLL_M = 0x28;// 0xb18
            clock = (clock * 8 + 5) / 10;
            break;
        case 6://12 bit
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_DIV, 0x00);//Addr = 0x9C24; *Addr = 0x00;
            ...
            if (IsTMDSLBR(phyindex) == TRUE) {
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_DIV, 0xC2);//Addr = 0x9C20; *Addr = 0xC2;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_HI, 0x1E);//Addr = 0x9C21; *Addr = 0x1E;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_MI, 0x00);//Addr = 0x9C22; *Addr = 0x00;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_LO, 0x00);//Addr = 0x9C23; *Addr = 0x00;
            }
            else {
        ...
        case 7://16 bit
            ...
            SC_SetByteDPppPHY(phyindex, _DPPHY_FA2_FREQ_LO, 0x00);//Addr = 0x9C27; *Addr = 0x00;
            if (IsTMDSLBR(phyindex) == TRUE) {
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_DIV, 0xC6);//Addr = 0x9C20; *Addr = 0xC6;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_HI, 0x14);//Addr = 0x9C21; *Addr = 0x14;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_MI, 0x00);//Addr = 0x9C22; *Addr = 0x00;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_LO, 0x00);//Addr = 0x9C23; *Addr = 0x00;
            }
            else {
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_DIV, 0xC4);//Addr = 0x9C20; *Addr = 0xC4;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_HI, 0x14);//Addr = 0x9C21; *Addr = 0x14;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_MI, 0x00);//Addr = 0x9C22; *Addr = 0x00;
                SC_SetByteDPppPHY(phyindex, _DPPHY_FA1_FREQ_LO, 0x00);//Addr = 0x9C23; *Addr = 0x00;
            }
            SC_DVI_PLL_M = 0x40;// 0xb18
            clock = (clock * 8 + 8) / 16;
            break;
    }
    return clock;
...
//        endvol = (ULONG)value * 0xFFFE / HDMI_AUDIO_VOLUME_RANGE + 1;
        endvol = (ULONG)value * 0x7FFF / HDMI_AUDIO_VOLUME_RANGE + 1;//range 1 ~ 0x8000
        ConfigHDMIAnalogAudioGain(DEF_HDMI_ANA_VOL);
        if (endvol > 0x6000) {
...
{
//    SC_AUDIO_OUT_CTRL = AUDIO_OUT_CTRL | BIT1;
//    SC_AUDIO_ANALOG_OUT_CTRL = 0xFC;
}

void ScalerAudioPreMute(void)
{
#if ENABLE_SCALER_AUDIO == ON
    SC_AUDIO_SMOOTH = 0x81;
    SC_AUDIO_VOLUME_R = 0xC6;
    SC_AUDIO_VOLUME_L = 0xC6;
    usTimerANAMute = GetTimerTick();
#endif
}
/**************************************************************************//**
...
void ScalerAudioSelect(AudioInOutChannel ch)
{
    ...
    switch(ch & AUDIO_IN_CH_MASK) {
        case AUDIO_IN_LINE1:
            ...
            SC_AUDIO_AUTO_CTRL_8B0 = 0x00;
            SC_AUDIO_AUTO_CTRL_8B3 = 0x01;
            SC_AUDIO_DELTA_SIGMA_CTRL &= ~(BIT6 | BIT5);
            break;
        case AUDIO_IN_LINE2:
        #if ENABLE_HDMI_AUDIO_AUTO_MUTE == ON
        ...
        default : // AUDIO_IN_HDMI
            ...
            SC_AUDIO_AUTO_CTRL1 = 0x03;
        #endif
            SC_AUDIO_SELECT = (SC_AUDIO_SELECT & 0xCF) | BIT5;  // 0x71a
            SC_AUDIO_AUTO_CTRL_8B0 = 0x30;  // 0x8b0
            SC_AUDIO_AUTO_CTRL_8B3 = 0x01;//0xFF;
            SC_AUDIO_DELTA_SIGMA_CTRL |= (BIT6 | BIT5); // 0x71d
    ...
    switch(ch & AUDIO_OUT_CH_MASK) {
        ...
        default : // AUDIO_OUT_LINEOUT
            SC_AUDIO_SELECT = (SC_AUDIO_SELECT & 0xF3);  // 0x71a
            break;
    }
    // Enable external cap for improving audio performance
    SC_AUDIO_DAC_DRIV_CTRL = 0xCC; //test by Rex 0x00        0x719
    SC_AUDIO_ANALOG_OUT_CTRL = 0xFC;//[P3]vcm run   // 0x724
}
/**************************************************************************//**
...
{
    if (((SC_HDMI_DET & BIT1) != 0) && (SC_HDMI_AVI_TYPE == 0x82) && ((SC_HDMI_AVI[0] & 0x60) != 0)) {//HDMI & YUV color// 0x734
        return TRUE;
    }
    else {
        return FALSE;
