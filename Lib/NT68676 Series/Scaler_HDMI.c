...
void HDMIInit(void)
{
    ...
    SC_AUDIO_VOLUME_R = 0x3B;
    SC_AUDIO_VOLUME_L = 0x3B;
    ScalerAudioPowerOn();

    if (SC_REVISION_ID != 0) { //E version
 ...
 * @param value The range is from 0x00 to 0x3B.
 ******************************************************************************/
static void ConfigHDMIAnalogAudioGain(UCHAR value)
{
...
#define VOL_STEP 8
    ...
    if (((SC_AUDIO_SELECT & 0x20) != 0) && (SC_REVISION_ID != 0)) { //PCM & E version
        ...
        value = (ULONG)value * 0x3B / HDMI_AUDIO_VOLUME_RANGE;
...
/**************************************************************************//**
 * Finetune the common level of audio output.
 * Throuhg this function, we can improve Popo noise issue over NT68676.
 ******************************************************************************/
static void AudioOffset(void)
{
#define FREQ 1000
    UCHAR offset, temp1, temp2, temp3;
    USHRT lvol, rvol;
    ULONG ratio;

    temp1 = SC_INPUT_SELECT;
    SC_INPUT_SELECT = 0;
    ratio = (float) FREQ * 16384 * 131072 / REF_CLK;
    Write3ByteScaler(&SC_VCLK_RATIO_LO, ratio);
    ucReg0x8605 = SC_HDMI_SOFTWARE_RESET;
    ...
    temp2 = SC_AUDIO_AUTO_OFFSET_CTRL;
    temp3 = SC_HDMI_MUTE;
    SC_AUDIO_ANALOG_OUT_CTRL = 0x40;
    SC_HDMI_MUTE |= BIT1;
    SC_AUDIO_AUTO_OFFSET_CTRL |= BIT7;
    lvol = ReadShortScaler(&SC_DIG_AUDIO_GAIN_L_LO);
    rvol = ReadShortScaler(&SC_DIG_AUDIO_GAIN_R_LO);
    WriteShortScaler(&SC_DIG_AUDIO_GAIN_L_LO, 0x0001);
    WriteShortScaler(&SC_DIG_AUDIO_GAIN_R_LO, 0x0001);
    SC_DIG_L_OFFSET = 0x00;
    SC_DIG_R_OFFSET = 0x00;
    Sleep(2);
//Search L channel
    if ((SC_AUDIO_AUTO_OFFSET_CTRL & BIT1) != 0) {
        for (offset=0xFF; offset>0x7F; offset--) {
            SC_DIG_L_OFFSET = offset;
            Sleep(2);
            if ((SC_AUDIO_AUTO_OFFSET_CTRL & BIT1) == 0) {
                break;
            }
        ...
        for (offset=0x00; offset<0x80; offset++) {
            SC_DIG_L_OFFSET = offset;
            Sleep(2);
            if ((SC_AUDIO_AUTO_OFFSET_CTRL & BIT1) != 0) {
                break;
...
//Search R channel
    if ((SC_AUDIO_AUTO_OFFSET_CTRL & BIT0) != 0) {
        for (offset=0xFF; offset>0x7F; offset--) {
            SC_DIG_R_OFFSET = offset;
            Sleep(2);
            if ((SC_AUDIO_AUTO_OFFSET_CTRL & BIT0) == 0) {
                break;
            }
    ...
    if ((SC_DIG_L_OFFSET == 0x7F) || (SC_DIG_L_OFFSET == 0x80)) {
        SC_DIG_L_OFFSET = 0x00;
    }
    if ((SC_DIG_R_OFFSET == 0x7F) || (SC_DIG_R_OFFSET == 0x80)) {
        SC_DIG_R_OFFSET = 0x00;
    }

    SC_INPUT_SELECT = temp1;
    SC_HDMI_SOFTWARE_RESET = ucReg0x8605;
    SC_AUDIO_SELECT = ucReg0x871A;
    SC_AUDIO_DELTA_SIGMA_CTRL = ucReg0x871D;

    WriteShortScaler(&SC_DIG_AUDIO_GAIN_L_LO, lvol);
    WriteShortScaler(&SC_DIG_AUDIO_GAIN_R_LO, rvol);
    SC_AUDIO_AUTO_OFFSET_CTRL = temp2;
    SC_HDMI_MUTE = temp3;
#undef FREQ
}
