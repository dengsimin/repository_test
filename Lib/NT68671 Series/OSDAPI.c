...
void OSDAPI_SetOSDWinAttr(OSDWinAttr attr)
{
    ...
    if (CHK_WINATTR_BEVEL(attr) || CHK_WINATTR_SHADOW(attr)) {
    ...
    SC_OSD_WIN_BEVEL_WIDTH  = (attr.ucBorderAttr & 0x70);  // Bevel Type
...
{
    if (times == 0) {
        SC_OSD_ROW_ATTRIBUT_CTRL1 &= ~(BIT1|BIT0);
    } 
    else {
        SC_OSD_ROW_ATTRIBUT_CTRL1 |= (BIT1|BIT0);
