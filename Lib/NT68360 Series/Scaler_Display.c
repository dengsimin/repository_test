...
void LoadDefaultPanelData(void)
{
    ...
    SC_LVDS_CTRL |= BIT1;
    SC_DISP_INTERFACE_CTRL = (PANEL_TEN_BIT_SHIFT << 6) | (PANEL_FOUR_PORT_SWAP << 2);
#else
    SC_LVDS_CTRL &= ~BIT1;
#endif
#endif
...
BOOL SetupDisplayTiming(USHRT htotal50)
{
    ...
    if (CheckModeChangeFlag() == FALSE) {
        if (interlace == TRUE) { //1080i
            ...
            i = ReadShortScalerHighLowFilter(&SC_DSP_HTOTAL_LO) & 0xFFFF;
...
#if ((ENABLE_SCALER_3D == ON) && (ENABLE_SHUTTER_3D == ON))
        if ((SC3DAPI_IsHDMI3DMode() == TRUE) && ((SC_HDMI_3D_FORMAT & 0xF0) == TOP_BOTTOM)) { //If input is not HDMI 3D format, then exit directly
            i = UserPanelData.usMinVSyncWidth + UserPanelData.usMinVSyncBackPorch;
        }
        else {
...
{
    SC_GAMMA_CTRL |= (BIT4|BIT3);    //setting display back end dither 
    SC_DITHER_TOGGLE_CTRL |= BIT7;
    SC_GAMMA_CTRL &= ~BIT3;    //setting display front end dither 
}
...
*/
    ...
    //        SC_DISPLAY_MUTE_CTRL = 0x00;
        ...
        #if ENABLE_SCALER_3D == ON
                if (SC3DAPI_IsHDMI3DMode() && (SC3DAPI_GetHDMI3DFormat() != FRAME_PACKING)) {
                    // When the input timing is 3D interlace, except Frame Packing, we must set manual
                    // 3D mode first to make the following odd/even adjustment function correct.
                    // XXX Need confirm this with designer.
                    SC_SCALER_3D_DEBUG1 |= BIT7;
                    SC_SCALER_3D_DEBUG1 &= ~BIT4;
                }
                else {
