...
//******************************************************************************
#define SetHDMI3DFlag() (ucScaler3DStatus |= BIT0)  /*!< A flag for HDMI 3D timing. */
#define IsHDMI3DFlag()  (ucScaler3DStatus & BIT0)   /*!< TRUE if input is HDMI 3D else FALSE. */
#define SetAMD3DFlag()  (ucScaler3DStatus |= BIT1)  /*!< A flag for AMD 3D timing. */
#define IsAMD3DFlag()   (ucScaler3DStatus & BIT1)   /*!< TRUE if input is AMD 3D else FALSE. */
#define Set2DTo3DFlag() (ucScaler3DStatus |= BIT2)  /*!< A flag for 2D to 3D. */
#define Is2DTo3DFlag()  (ucScaler3DStatus & BIT2)   /*!< TRUE if display is 2D to 3D else FALSE. */
#define Set2DToFSFlag() (ucScaler3DStatus |= BIT3)  /*!< A flag for 2D to frame sequence. */
#define Is2DToFSFlag()  (ucScaler3DStatus & BIT3)   /*!< TRUE if display is 2D to frame sequence else FALSE. */
#define SetSBSFlag()    (ucScaler3DStatus |= BIT4)  /*!< A flag for SBS display. */
#define IsSBSFlag()     (ucScaler3DStatus & BIT4)   /*!< TRUE if display is SBS else FALSE. */
#define SetTBFlag()     (ucScaler3DStatus |= BIT5)  /*!< A flag for TB display. */
#define IsTBFlag()      (ucScaler3DStatus & BIT5)   /*!< TRUE if display is TB else FALSE. */
#define Set3DTo2DFlag() (ucScaler3DStatus |= BIT7)  /*!< A flag for 3D to 2D. */
#define Clr3DTo2DFlag() (ucScaler3DStatus &= ~BIT7) /*!< Clear the 3D to 2D flag. */
#define Is3DTo2DFlag()  (ucScaler3DStatus & BIT7)   /*!< TRUE if display is 3D to 2D else FALSE. */
#define ClrAll3DFlag()  (ucScaler3DStatus &= 0x80)  /*!< Clear all flags. */
#define Is3DFlag()      (ucScaler3DStatus & 0x3F)   /*!< TRUE if display is 3D. */
...
//******************************************************************************
static xdata UCHAR ucHDMI3DFormat;   /*!< Variable for 3D format. If input is 2D, the value will be 0xFF */
static xdata UCHAR ucScaler3DStatus; /*!< Variable for 3D flags. */
static BOOL bDyn2DTo3DFlag = 0;      /*!< TRUE if input content is colorful image else FALSE. */
static xdata ULONG glb_level;        /*!< Variables for 2D to 3D functions. */
static xdata ULONG glb_max_0;        /*!< Variables for 2D to 3D functions. */
static xdata ULONG glb_max_1;        /*!< Variables for 2D to 3D functions. */
static xdata ULONG glb_max;          /*!< Variables for 2D to 3D functions. */
static xdata ULONG glb_counter;      /*!< Variables for 2D to 3D functions. */
static xdata ULONG glb_reg_50;       /*!< Variables for 2D to 3D functions. */
static xdata ULONG glb_reg_51;       /*!< Variables for 2D to 3D functions. */
static xdata ULONG glb_reg_52;       /*!< Variables for 2D to 3D functions. */
...
{
    SC_POWER_CTRL2 |= BIT4;
    SC_FRC_CTRL = 0x00; //FRC off
    SC_PREFRC_CONTROL = 0x00; //turn off: pre-frc
    SC_3D_FORMAT_CTRL = 0x00;
    SC_OSD_3D_FORMAT = 0x00;
    SC_OSD_FRAME2_FUNC_CTRL = 0x00;
    SC_BIST_CTRL2 |= 0x02;
    ClrAll3DFlag();
}
...
/**************************************************************************//**
 * Initialize the registers related L/R output signal when 3D display is shutter glass type.
 ******************************************************************************/
...
/**************************************************************************//**
 * Select 3D L/R sync signal output port.
 * @param port L/R sync output port.
 ******************************************************************************/
...
/**************************************************************************//**
 * Set the high/low duty of L/R sync when output port is LOUT or ROUT.
 * The unit is 1H.
 * @param on Delay number for vertical lines to pull high L/R sync signal.
 * @param off Delay number for vertical lines to pull low L/R sync signal.
 ******************************************************************************/
void SC3DAPI_SetSyncDuty(USHRT on, USHRT off)
{
    SC_3D_SHUTTER_GLASS_CTRL[0x09] = (SC_3D_SHUTTER_GLASS_CTRL[0x0A] & 0xF0) + (on & 0x000F);
    SC_3D_SHUTTER_GLASS_CTRL[0x0A] = (UCHAR)(on >> 4);
    SC_3D_SHUTTER_GLASS_CTRL[0x09] = (SC_3D_SHUTTER_GLASS_CTRL[0x0A] & 0x0F) + (off & 0x000F) << 4;
    SC_3D_SHUTTER_GLASS_CTRL[0x0B] = (UCHAR)(off >> 4);
}

/**************************************************************************//**
 * Set the high/low duty of L/R sync when output port is TVIR.
 * The high/low duties are 50%/50%.
 * @param vsync L/R sync frequency.
 * @param delay Delay the percentage of total duty time. The range is 0~100.
 ******************************************************************************/
...
/**************************************************************************//**
 * Invert HDMI/AMD 3D L/R state.
 * @param inv TRUE if L/R inverted else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Enable 3D L/R detection from CTRL bits (for AMD 3D).
 * @param enable TRUE if AMD 3D L/R detection is enabled else FALSE
 ******************************************************************************/
 ...
 * @return TRUE if input is AMD 3D else FALSE.
 ******************************************************************************/
UCHAR SC3DAPI_DetectAMD3D(void)
{
    ...
    if (!IsHDMI()) {
        ...
        // Get first to clear the CTRL status bit.
...
/**************************************************************************//**
 * Detect AMD 3D flags(CTRL).
 * @return TRUE if AMD L/R flag is detected else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Down convert the input frame rate.
 * If input frame rate is greater than 62Hz, we convert the frame rate to 60Hz.
 * If input frame rate is 56Hz, we convert the frame rate to 50Hz.
 * We apply this function on 2D to 3D, 2D to TB and 2D to SBS to avoid internal clock over spec.
 ******************************************************************************/
static void SC3DAPI_AutoFrameDown(void)
{
    if (usVSyncFreq > 620) {
...
/**************************************************************************//**
 * Configure scaler clock and scaling factor when input is TB or SBS.
 * @param hfactor Horizontal scaling factor. The ratio is hfactor/2 which base factor is 2.
 * @param vfactor Vertical scaling factor. The ratio is vfactor/2 which base factor is 2.
 * @param ratio Scaler clock ratio. The ratio is ratio/10 which base factor is 10.
 ******************************************************************************/
static void SC3DAPI_ConfigScaling(UCHAR hfactor, UCHAR vfactor, UCHAR ratio)
{
    ULONG clock, tmpclk;
    ...
    // HTotal of 2D->3D is limited and must be larger than UserPanelData.usWidth + 216
    if (SC3DAPI_Is2DTo3D()) {
        if ((dsthtotal - 220) < UserPanelData.usWidth) {
            i = UserPanelData.usWidth + 220;
            clock = (ULONG) ((float) clock * i / dsthtotal);
            dsthtotal = i;
        }
    }
    ...
    // Enable FRC to make the post clock refer to B38~B3A.
    // This can avoid scaler FIFO setting refer to wrong clock value!
    SC_FRC_CTRL = 0x09;

    Write3ByteScaler(&SC_FRC_RATIO_LO, clock);
...
/**************************************************************************//**
 * Configure pre-FRC settings when input is LA.
 * @param hratio Horizontal scaling factor. The ratio is hfactor/2 which base factor is 2.
 * @param vratio Vertical scaling factor. The ratio is vfactor/2 which base factor is 2.
 ******************************************************************************/
static void SC3DAPI_ConfigPreFRC(UCHAR hratio, UCHAR vratio)
{
    USHRT htotal, vtotal;
...
/**************************************************************************//**
 * Configure post-FRC setting (output display timing).
 ******************************************************************************/
static void SC3DAPI_ConfigPostFRC(void)
{
...
#define FRC_VBI_RATIO 10 // This value must higher than 10
    ...
    if ((Is3DFlag() || IsInterlaced()) && (lock == TRUE)) {
        ...
        // Because the vtotal from SC_DV_TOTAL_LO may be smaller than the value for normal display over FRC lock mode,
        // we force the FRC vtotal as (SC_DV_TOTAL_LO + 10). (We guess 10 is enough and the test result is OK).
        //
        // And why we do not use the vtotal value from FRC(0x548:0x41)?
        // Because the delay time to read FRC-in vtotal is uncertain and it makes f/w flow complex.
        //
        // BTW, even we increase FRC output vtotal, the VSync frequency is nothing changed.
        // Because FRC is under LOCK mode.
        WriteShortScaler(&SC_FRC_OUTPUT_V_TOTAL_LO, vtotal + 10);
    }
    else {
        SC_FRC_CTRL = 0x13;
...
/**************************************************************************//**
 * According to the FRC/OD settings, we adapt the MMU fifo settings.
 ******************************************************************************/
static void SC3DAPI_ConfigFIFO(void)
{
...
/**************************************************************************//**
 * According to the post-FRC settings, we adapt the post-FRC lock position.
 ******************************************************************************/
static void SC3DAPI_ConfigFRCLock(void)
{
    USHRT hlock, vlock;
...
/**************************************************************************//**
 * According input signal, we adapt FRC/3D related registers.
 ******************************************************************************/
void SC3DAPI_Setup3DMode(void)
{
    code UCHAR scaling_vector[9][3] = {
        {2, 2, 10}, //FRAME_PACKING
        {2, 2, 10}, //FIELD_ALTERNATIVE
        {2, 2, 20}, //LINE_ALTERNATIVE
        {4, 2, 20}, //SIDE_BY_SIDE_FULL
        {2, 2, 10}, //LDEPTS
        {2, 2, 10}, 
        {2, 4, 20}, //TOP_BOTTOM
        {2, 2, 10}, 
        {4, 2, 20}  //SIDE_BY_SIDE_HALF
    };
#if ENABLE_SHUTTER_3D == OFF //retarder panel
    code UCHAR retarded_ctl[9] = {0xF4, 0xF4, 0xF4, 0xFC, 0x20, 0x20, 0xF4, 0x20, 0xFC};
#endif
    ...
    if (SC3DAPI_IsHDMI3DMode() == FALSE) { //If input is not HDMI 3D format, then exit directly
        if (SC3DAPI_DetectAMD3D()) {
            SC_SCALER_3D_DEBUG1 = 0x80;
            type = (FRAME_PACKING>>4)&0x0F;
            SC_3D_FORMAT_SEL = (FRAME_PACKING>>4)&0x0F;
            SC_3D_FORMAT_CTRL |= BIT0;
            SetAMD3DFlag();
        }
#if ENABLE_3D_CNV == ON
        else if (UserData.uc2DTo3DMode == THR_D_CNV_SIDEBYSIDE) {
            SC3DAPI_AutoFrameDown();
            SC_SCALER_3D_DEBUG1 = 0x83;
            type = (SIDE_BY_SIDE_HALF>>4)&0x0F;
            SC_3D_FORMAT_SEL = (SIDE_BY_SIDE_HALF>>4)&0x0F;
            SC_3D_FORMAT_CTRL |= BIT0;
            SetSBSFlag();
        }
        else if (UserData.uc2DTo3DMode == THR_D_CNV_TOPBOT) {
            SC3DAPI_AutoFrameDown();
            SC_SCALER_3D_DEBUG1 = 0x96;
            type = (TOP_BOTTOM>>4)&0x0F;
            SC_3D_FORMAT_SEL = (TOP_BOTTOM>>4)&0x0F;
            SC_3D_FORMAT_CTRL |= BIT0;
            SetTBFlag();
        }
        else if (UserData.uc2DTo3DMode == THR_D_CNV_FRAME_SEQ) {
            SC_SCALER_3D_DEBUG1 = 0x80;
            type = (FRAME_PACKING>>4)&0x0F;
            SC_3D_FORMAT_SEL = 0x09;
            SC_3D_FORMAT_CTRL |= (BIT6 | BIT0);
            Set2DToFSFlag();
        }
        else if (UserData.uc2DTo3DMode == THR_D_CNV_2D_TO_3D) {
#if ENABLE_SHUTTER_3D == OFF //retarder panel
            SC_BUF_LESS_MODE |= (BIT0|BIT1);
#endif
            SC3DAPI_AutoFrameDown();
            SC3DAPI_Enable2DTo3D(TRUE);
        ...
        switch (SC_HDMI_3D_FORMAT & 0xF0) { //Check 3D Format
            ...
            case LINE_ALTERNATIVE:
                SC_3D_FORMAT_SEL = LINE_ALTERNATIVE;
                SC3DAPI_ConfigPreFRC(2, 1);
                break;
            case SIDE_BY_SIDE_FULL:
            case SIDE_BY_SIDE_HALF:
                SC_3D_FORMAT_SEL = (SC_HDMI_3D_FORMAT>>4)&0x0F;
                SetSBSFlag();
                break;
            case TOP_BOTTOM:
                SC_3D_FORMAT_SEL = (SC_HDMI_3D_FORMAT>>4)&0x0F;
    ...
    SC3DAPI_ConfigPostFRC();
    SC3DAPI_ConfigFRCLock();
    SC3DAPI_ConfigFIFO();

    SC_FRC_OUTPUT_VS_WIDTH_LO = (64 - (PANEL_WIDTH & 0x3F)) & 0x3F; //PostFRC Residual

    if (Is3DFlag()) {
    #if ENABLE_SHUTTER_3D == OFF
        WriteShortScaler(&SC_OSD_REF_F_PPOCH_LO, (SC_FRC_V_FRONT_PORCH_LSB
                | ((SC_FRC_VH_FRONT_PORCH_MSB & 0xf0) << 4)) + (SC_DV_BG_BEG_LO
                | (SC_DV_BG_BEG_HI << 8))); // re-calculate OSD V start position if swap OSD

        if ((PANEL_WIDTH & 0x3f) != 0) { //PostFRC Residual issue (x64), by YL and Boren
            SC_FRC_OUTPUT_VS_WIDTH_LO |= BIT7;
        }

        SC_MICRO_RETARDED_CTRL = retarded_ctl[type];
    #endif

        SC_OSD_FRAME2_FUNC_CTRL &= ~BIT3;
        if (((type << 4) & 0xF0) == LINE_ALTERNATIVE) {
            SC_OSD_3D_FORMAT = 0x00;
        }
        else {
            SC_OSD_3D_FORMAT = ((type << 4) & 0xF0) | BIT3;
        }

        if ((((SC_3D_FORMAT_SEL & 0x0F) << 4) == SIDE_BY_SIDE_FULL) ||
            (((SC_3D_FORMAT_SEL & 0x0F) << 4) == SIDE_BY_SIDE_HALF)) {
            SC_SCALER_3D_DEBUG1 = 0x80;
            SC_INTE_CTRL = 0x02;
        }
        else {
 ...
 * @param enable Enable or disable 3D to 2D function.
 ******************************************************************************/
...
void SC3DAPI_TurnOff3DMode(void)
{
    ...
    // We must delay here because of the Vsync double buffer of FRC bypass circuit.
    WaitDisVSync();
    SC_POWER_CTRL2 &= ~BIT4;
    ...
    SC_OD_FIFO_CTRL1 = 0x1D;
    SC_OD_FIFO_CTRL2 = 0x1F;
    SC_OD_FIFO_CTRL3 = 0x1D;
...
#if ENABLE_3D_CNV == ON
/**************************************************************************//**
 * Turn on/off 2D to 3D function.
 * @param enable TRUE or FALSE to enable/disable 2D to 3D function.
 ******************************************************************************/
...
/**************************************************************************//**
 * Set 2D to 3D view point.
 * @param vpoint Value of view point. Range is 0 ~ 11.
 ******************************************************************************/
...
/**************************************************************************//**
 * Set 2D to 3D depth level.
 * @param dlvl Value of depth level. Range is 0 ~ 10.
 ******************************************************************************/
...
/**************************************************************************//**
 * Set the size of left and right borders automatically.
 ******************************************************************************/
...
/**************************************************************************//**
 * Update luminance information.
 * This function is used for adaptive 2D to 3D function.
 ******************************************************************************/
...
/**************************************************************************//**
 * According to the luminance information, we update 2D to 3D depth level automatically.
 * This function is used for adaptive 2D to 3D function.
 * @param on TRUE to fix the depth level to maximum else FALSE.
 ******************************************************************************/
...
#define MIN_OFFSET_51 ((TOTAL_TH)/2)
    ...
    if ((max > MAX_TH)&&(bDyn2DTo3DFlag)) {
        ClassLevel = 2;
    }
    else if (((max < MAX_TH)&&(max > MIN_TH))||((!bDyn2DTo3DFlag)&&(max > MAX_TH))) {
        ClassLevel = 1;
    }
    else {
        ClassLevel = 0;
...
/**************************************************************************//**
 * Check input timing is HDMI 3D or not by the raw VSI packet.
 * @return TRUE if HDMI 3D mode else FALSE.
 ******************************************************************************/
static BOOL SC3DAPI_CheckHDMI3DInfoFrame(void)
{
    UCHAR tmpreg;
...
/**************************************************************************//**
 * Check input timing is HDMI 3D or not by scaler VSI-related registers.
 * Note: If VSI is not present, the registers will keep same as the values scaler got last time.
 * @return TRUE if HDMI 3D mode else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check input timing is HDMI 3D or not by scaler 3D status registers.
 * Note: If VSI is not present, the registers will keep same as the values scaler got last time.
 * @return TRUE if HDMI 3D mode else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check HDMI 3D format is changed or not.
 * @return TRUE if HDMI 3D mode is changed else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Configure the HDMI 3D related registers.
 * Through this function, scaler will turn off auto-3D function.
 * Through this function, scaler will double the frame rate when input is FP or FA.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check the active space is changed or not.
 * @return TRUE if active space is changed else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Swap L/R frame
 * @param swap TRUE to swap the LR else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check current display is HDMI 3D or not.
 * @return TRUE if current display is HDMI 3D else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check current display is AMD 3D or not.
 * @return TRUE if current dispaly is AMD 3D else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check current display is 2D to 3D or not.
 * @return TRUE if current display is 2D to 3D else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check current display is FS (frame sequence) or not.
 * @return TRUE if current display is FS else FALSE.
 ******************************************************************************/
/*
...
/**************************************************************************//**
 * Check current display is SBS or not.
 * @return TRUE if current display is SBS else FALSE.
 ******************************************************************************/
/*
...
/**************************************************************************//**
 * Check current display is TB or not.
 * @return TRUE if current display is TB else FALSE.
 ******************************************************************************/
/*
...
/**************************************************************************//**
 * Check current display is 3D or not.
 * @return TRUE if current display is 3D else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check current display is 3D to 2D or not.
 * @return TRUE if current display is 3D to 2D else FALSE.
 ******************************************************************************/
/*
...
/**************************************************************************//**
 * Check input timing is 3D format or not.
 * If TRUE, this function will force the timing as manual HDMI 3D.
 * This function check timing format, not VSI packet.
 * We use this function only when there is no 3D information inside the info-frame.
 * @return TRUE if the timing format is 720P FP or 1080P FP else FALSE.
 ******************************************************************************/
...
/**************************************************************************//**
 * Check the current input timing is manual HDMI 3D or not.
 * We can not call this function until SC3DAPI_Is3DFormat is performed.
 * @return TRUE if the timing is manual HDMI 3D else FALSE.
 ******************************************************************************/
