...
void WriteHDCPKey(UCHAR *pKSV, UCHAR *pKey)
{
    ...
    SC_HDCP_INPUT_CTRL = 0x80 | HDCP_CLK_INV;  // Set Refclk * clk inverted
    ...
    for (i = 0; i < 282; i++) {
        ...
        tmp_reg = pKey[i];
#endif
        SC_INDEX_ADDR_LO = (UCHAR)(i & 0xFF);
        SC_INDEX_ADDR_HI = (UCHAR)(i >> 8);
        SC_INDEX_DATA = tmp_reg;
    }
...
{
    SC_HDCP_INPUT_CTRL = 0x00 | HDCP_CLK_INV;
    SC_HDCP_SLAVE_ADDR = 0x74;
#if ENABLE_DVI_DUAL_LINK == ON    
    SC_HDCP_SLAVE_ADDR2 = 0x76;
#endif
}
