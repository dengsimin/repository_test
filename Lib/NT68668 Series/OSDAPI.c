...
void OSDAPI_SetOSDWinAttr(OSDWinAttr attr)
{
    ...
    SC_OSD_WIN_BEVEL_WIDTH = attr.ucBorderAttr & 0x07; // Bevel Width
...
#else
    SC_OSD_WIN_GRADIENT_CTRL1 = 0x00;
    OSDAPI_TurnOnOffOSDWinGrad(ON);
    SC_OSD_WIN_GRADIENT_CTRL1 |= ((dir&BIT0)<<4);
    if (inc)
        SC_OSD_WIN_GRADIENT_CTRL1 |= 0xE0;
    else
        SC_OSD_WIN_GRADIENT_CTRL1 &= ~0xE0;
    SC_OSD_WIN_GRADIENT_CTRL2 = ((pixel & 0x0F) << 4) | step;
#endif
}
...
#else
    if (on) {
        SC_OSD_WIN_GRADIENT_CTRL1 |= 0x0E;
    }
    else {
        SC_OSD_WIN_GRADIENT_CTRL1 &= 0xF1;
    }
#endif
}
...
#else
        dir = !(SC_OSD_WIN_GRADIENT_CTRL1 & BIT4);
        SC_OSD_WIN_GRADIENT_CTRL1 &= ~BIT4;
        SC_OSD_WIN_GRADIENT_CTRL1 |= (dir << 4);
#endif
    }
