...
void SyncInit(void)
{
    ...
    SC_SOG_DIGITAL_GLITCH_FILTER_THREESHOLD = SCREG_00B;
#endif
}
...
{
    //Detect VGA H/V sync 
    SC_ADC_ZERO_POWER_CTRL = 0x8B;    //enable CR R/W, SOG power
    SC_INPUT_SELECT = 0x00;//select DSUB
    SC_SOG_SLICER_CTRL |= (BIT2 | BIT0);    //SOG slicer en
    ...
    if (((ucZpInterface==ANALOG_SEP_INPUT0) || (ucZpInterface==ANALOG_COM_INPUT0)) && ((HS_CNT_LB>10) || (HS_CNT_HB>0))) {
        ...
        if ((ucZpInterface==ANALOG_SEP_INPUT0) && ((VS_CNT>0) && (VS_CNT<10))) { //Vsync detected
            ...
            SC_ADC_CTRL = 0x13; //Select HSYNCI
...
#if IS_NT68660_SERIES || IS_NT68650_SERIES
/**************************************************************************//**
 * Turn on H/V sync filter.
 ******************************************************************************/
void HVFilterOn(void)
{
    SC_SOG_DIGITAL_GLITCH_FILTER_THREESHOLD = SCREG_00B;
}

/**************************************************************************//**
 * Turn off H/V sync filter.
 ******************************************************************************/
void HVFilterOff(void)
{
...
{    
    BOOL result=FALSE;
    UCHAR Reg0xB57, Reg0x102, Reg0x101, Reg0x333, Reg0x335, Reg0x336;    
    UCHAR Reg0x302;
