...
{   
    ...
    if ((dlg == OSD_DLG_OSDUNLOCKKEY)||(dlg == OSD_DLG_REMINDER)
        ...
        ||(dlg == OSD_DLG_NOCABLE)
        ||(dlg == OSD_DLG_OSDLOCK)
        ||(dlg == OSD_DLG_NONPRESET)
        ||(dlg == OSD_DLG_RESNOTE)
        ||(dlg == OSD_DLG_OUTOFRANGE)
        ) {
...
#endif            
        {
            #if (ModelName == BENQ_BL2720PT_DEMO_FD)            
                OSDSetPosition(1600,800);
            #else
...
//        OSDClearDialogSpaceFrame(0,OSD_DIALOG_HEIGHT,OSD_DIALOG_SELECTPAGE_WIDTH,OSD_DIALOG_SELECTPAGE_HEIGHT);    
...
#endif            
        {
            OSDSetPosition(OSD_RESET_H_POSITION,OSD_RESET_V_POSITION);
        }
        ...
        OSDSetPosition(171,0);
        LogoSetSplit((OSD_SENSEYE_DEMO_WIDTH/2),0,(PANEL_WIDTH/12/2-OSD_SENSEYE_DEMO_WIDTH/2),0);
    }        
    else{
    ...
    switch (dlg) {
        case OSD_DLG_BURNIN:
            ...
            ShowIndentString(TRUE, 6, 3, 20, BurningStrTab[UserData.ucLanguage]);   
            break;
        ...
        case OSD_DLG_SENSEYE_DEMO:
            ...
            OSDAPI_SetFontAttr(NoticeStringColor);
            //ShowIndentString(TRUE, 0, 0, 13, SenseyeOnOffTab[OFF]);
            ShowIndentString(TRUE, 0, 0, (OSD_SENSEYE_DEMO_WIDTH/2), PictureModeTab11[UserData.ucBFMode*NUM_OSDLANGS + UserData.ucLanguage]); 
            ...
            ShowIndentString(TRUE, (OSD_SENSEYE_DEMO_WIDTH/2-1), 0, (OSD_SENSEYE_DEMO_WIDTH/2), DualViewTab11[UserData.ucDualViewMode*NUM_OSDLANGS + UserData.ucLanguage]); 
            //ShowIndentString(TRUE, OSD_SENSEYE_DEMO_WIDTH-13, 0, 13, SenseyeOnOffTab[ON]);
            OSDAPI_SetHVZoom(2);
            OSDAPI_SetAlignType(OSD_ALIGN_LEFT);
            break;        
        default:
...
void OSDShowSelectPageButton_Right(UCHAR id, UCHAR act, BOOL Clr)
{
    ...
    switch (id) {
        case 0:
            if (Clr) {        //Up	                  
               OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT,  BTN_Width1, SELECTPAGE_2BIT_COLOR1);    //BTN_width = 8  
                for (i=1; i<4; i++) {   
                    OSDAPI_FillLineAttr(Offset_x, Offset_y+id*BTN_HEIGHT+i,  1, TOPPAGE_2BIT_COLOR1); 
                    OSDAPI_FillLineAttr(Offset_x+1, Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, SELECTPAGE_1BIT_COLOR1_WHITE_B1);     
                }          
                OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT+i,  1, SELECTPAGE_4BIT_COLOR1);    //BTN_width = 8    
                OSDAPI_FillLineAttr(Offset_x+1,Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, SELECTPAGE_2BIT_COLOR3);    //BTN_width = 8*/
            }
            else {               
                OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT,  BTN_Width1, SELECTPAGE_2BIT_COLOR1_BLUE);    //BTN_width = 8  
                for (i=1; i<4; i++) {   
                    OSDAPI_FillLineAttr(Offset_x, Offset_y+id*BTN_HEIGHT+i,  1, TOPPAGE_2BIT_COLOR2_BLUE); 
                    OSDAPI_FillLineAttr(Offset_x+1, Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, SELECTPAGE_1BIT_COLOR1_BLACK_B1);     
                }          
                OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT+i,  1, SELECTPAGE_4BIT_COLOR_BLUE_UPPER);    //BTN_width = 8    
                OSDAPI_FillLineAttr(Offset_x+1,Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, TOPPAGE_2BIT_COLOR3_BLUE);    //BTN_width = 8
            }
            break;
        case 1:          //Down
            if(Clr) {                 
                OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT,  1, SELECTPAGE_4BIT_COLOR1);    //BTN_width = 8  
                OSDAPI_FillLineAttr(Offset_x+1,Offset_y+id*BTN_HEIGHT, BTN_Width1-1, TOPPAGE_2BIT_COLOR4);    //BTN_width = 8
                for (i=1; i<4; i++) {   
                OSDAPI_FillLineAttr(Offset_x, Offset_y+id*BTN_HEIGHT+i,  1, TOPPAGE_2BIT_COLOR1); 
                OSDAPI_FillLineAttr(Offset_x+1, Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, SELECTPAGE_1BIT_COLOR1_WHITE_B1);     
                }          
                ...
                OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT,  1, SELECTPAGE_4BIT_COLOR_BLUE_LOWER);    //BTN_width = 8  
                OSDAPI_FillLineAttr(Offset_x+1,Offset_y+id*BTN_HEIGHT, BTN_Width1-1, TOPPAGE_2BIT_COLOR1_BLUE);    //BTN_width = 8
                for (i=1; i<4; i++) {   
                OSDAPI_FillLineAttr(Offset_x, Offset_y+id*BTN_HEIGHT+i,  1, TOPPAGE_2BIT_COLOR2_BLUE); 
                OSDAPI_FillLineAttr(Offset_x+1, Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, SELECTPAGE_1BIT_COLOR1_BLACK_B1);     
                }          
        ...
        case 4:        //exit
            if (Clr)  {
                ...
                OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT+i,  1, TOPPAGE_2BIT_COLOR1);    //BTN_width = 8    
                OSDAPI_FillLineAttr(Offset_x+1,Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, SELECTPAGE_2BIT_COLOR1);    //BTN_width = 8
               ...
               OSDAPI_FillLineAttr(Offset_x,Offset_y+id*BTN_HEIGHT+i,  1, TOPPAGE_2BIT_COLOR2_BLUE);    //BTN_width = 8    
               OSDAPI_FillLineAttr(Offset_x+1,Offset_y+id*BTN_HEIGHT+i, BTN_Width1-1, SELECTPAGE_2BIT_COLOR1_BLUE);    //BTN_width = 8
            }
            break;
        }
...
void OSDShowLayer3PageHookIcon(UCHAR pageNum)
{
    ...
    if (ucMenuType == OSD_MAIN_SYSTEM) {
        ...
        else if (ucSubMenuType == OSD_PICTURE_AD_SENSEYE ) {//Justin 140428 CustomerKey1
		    ItemNum = UserData.bSensEyeDemo;//UserData.ucBFMode;
        }        
	}
...
BOOL isOSDClearLayer4(UCHAR ItemID)
{
    ...
    if(ucMenuType == OSD_MAIN_SYSTEM && ucSubMenuType == OSD_SYS_OSDSET && ItemID<2)//for Language/OSD Timer
        ...
        return TRUE;
    else if(ucMenuType == OSD_MAIN_PICTURE_ADVANCED && ucSubMenuType == OSD_PICTURE_AD_PIC 
#if ENABLE_PICTURE_MODE_ANIMATION == ON        
        && ItemID == PICTURE_MODE_ANIMATION
#endif        
        )//for LBL Menu
        return TRUE; 
...
{
    if (FALSE 
        ||(UserData.ucBFMode==PICTURE_MODE_STANDARD)
...
{
#if ENABLE_PICTURE_DUAL_MODE == OFF
    if ( FALSE
        ||(UserData.ucBFMode==PICTURE_MODE_SRGB)
        ||(UserData.ucBFMode==PICTURE_MODE_LOW_BL)       
        ||(UserData.ucBFMode==PICTURE_MODE_USER)
        ...
        ||(UserData.ucBFMode==PICTURE_MODE_DARKROOM)
#endif  
        )
    {
        return TRUE;
...
BOOL isOSDArabAlignRight(void)
{
    if(GetOSDState()>= OSD_SUBITEM_4_1_1_9_STATE&&GetOSDState()<= OSD_SUBITEM_4_1_1_14_STATE
    ||GetOSDState()==OSD_SUBITEM_4_1_1_16_STATE
    ||(GetOSDState()==OSD_ITEM_4_1_1_STATE&&Custom_System.ucLanguage!=OSDLANG_ARAB)){
        return FALSE;
    }
