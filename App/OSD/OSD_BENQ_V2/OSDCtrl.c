...
//******************************************************************************
BOOL bOSDShowAfterDisplay = FALSE;
        ...
        if (event != OSD_NONKEY_EVENT) {
            ...
            if (OSDKeyRoutine[usOSDState][event] != NULL) {
                ...
                if (usOSDState > OSD_TOPPAGE_BEG && usOSDState <= OSD_DIRECT_3_STATE){//ULEric140327N         
                   ...
                   if(event == OSD_EXITKEY_EVENT)
                   Custom_System.bPowerKeyYNPowerONOFFFlag = 1;
                }
                else if((usOSDState == OSD_TOPPAGE_BEG) && (event == OSD_EXITKEY_EVENT))
                   Custom_System.bPowerKeyYNPowerONOFFFlag = 1;
...
void OSDCancelPreview (USHRT OSDstate)
{
    ...
    if((OSDstate>=OSD_ITEM_3_2_1_STATE && OSDstate < OSD_ITEM_3_2_END) ||(OSDstate>=OSD_SUBITEM_3_2_1_STATE_BEG && OSDstate < OSD_SUBITEM_3_2_1_END) ){
    ...
    //Color format
    if(OSDstate>=OSD_ITEM_3_6_1_STATE && OSDstate < OSD_ITEM_3_6_END){
        SetColorSpace();            
    }
#endif    
    //HDMI Range
    if((OSDstate>=OSD_ITEM_3_7_1_STATE && OSDstate < OSD_ITEM_3_7_END)){
        SetColorSpace();            
    }
    //Mute
...
void OSDTimerRoutine(void)
{
    ...
    if ((usOSDState == OSD_OFF_STATE) && (GetModeHandleState()==WAIT_MODE_CHANGE) && (bOSDShowAfterDisplay == TRUE)) {
        if((UserData.bSensEyeDemo != PICTURE_DUALMODE_OFF) && (isOSDAllowSenseyeDemo())){
            if(Custom_System.bShowSensEyeDemoTemp == TRUE){
                usOSDLocalTimer = GetTimerTick();
                OSDShowSensEyeDemo();
            }
        }
	...
	else if (usOSDState == OSD_INFO_BURNING_STATE) {
        if ((GetTimerTick()-usOSDLocalTimer) > 100) {
            OSDMovePosition();
            usOSDLocalTimer = GetTimerTick();
        }
...
//                if((!IsNativeTiming())&&(IsShowResolutionNotice())&&(!IsHDMIVideoTiming())){
        ...
        if ((GetTimerTick()-usOSDLocalTimer) > 3000) {
            //OSDCloseAll();
            Custom_System.bShowSensEyeDemoTemp = FALSE;
...
void OSDCloseAll(void)
{
    Custom_System.bShowSensEyeDemoTemp = TRUE;
    OSDAPI_DisableSplit();
...
#else
    if (SystemData.bFactory) {//ULEric140331
#endif
        OSDCloseAll();        
        usOSDState = OSD_FACT_STATE;
...
void OSDItm32ExitTopPageMenu(void)
{
    ...
    if(UserData.ucDualViewMode !=Custom_System.ucDualViewTemp){
...
void OSDSubItm321ExitTopPageMenu(void)
{
    ...
    if(UserData.bSensEyeDemo != Custom_System.bShowSensEyeDemoTemp){
        UserData.bSensEyeDemo = Custom_System.bShowSensEyeDemoTemp;
    }
    if(UserData.ucDualViewMode !=Custom_System.ucDualViewTemp){
         UserData.ucDualViewMode  =  Custom_System.ucDualViewTemp;
    }
...
#endif
#if 1//ENABLE_CUSTOMKEY_ANIMATION == ON 
...
void OSDShowCustomerMenu(UCHAR Key_Setting) 
{
    ...
    switch (Key_Setting) {
        ...
        case CUSTOMER_KEY_INPUT:
            usOSDState = (OSD_CUSTKEY_INPUT_BEG+1) + GetInputPortIndex(UserGetInputSource());
...
void OSDEnterItm12(void)
{
    ...
    ucSubMenuType = CursorPos(OSD_MAIN_END,usOSDState);
    OSDFocusItm12();
    OSDSelectLayer3Item(CursorPos((OSD_ITEM_1_2_start_STATE+1),usOSDState));
}

void OSDExitItm12(void)
{
    //OSDConfirmItm12();
    OSDExitItms(CursorPos((OSD_ITEM_1_2_start_STATE+1),usOSDState));
...
{
    OSDUnSelectLayer3Item(CursorPos((OSD_ITEM_1_2_start_STATE+1), usOSDState), TRUE);
//    while (1) {
        usOSDState = PrevState((OSD_ITEM_1_2_start_STATE+1), OSD_ITEM_1_2_END, usOSDState);
...
{
    OSDUnSelectLayer3Item(CursorPos((OSD_ITEM_1_2_start_STATE+1), usOSDState), FALSE);
//    while (1) {
        usOSDState = NextState((OSD_ITEM_1_2_start_STATE+1), OSD_ITEM_1_2_END, usOSDState);
//        if (OSDIsSubItemValid(CursorPos(OSD_ITEM_1_2_1_STATE, usOSDState)) == TRUE)
//            break;
//    }
    OSDSelectLayer3Item(CursorPos((OSD_ITEM_1_2_start_STATE+1), usOSDState));
...
void OSDSelectInput(void)
{
    ...
    OSDindex = GetOSDState() - (OSD_ITEM_1_2_start_STATE+1);
    SoucerIndex = GetInputPortIndex(UserData.ucInputSource);
    if(OSDindex != SoucerIndex){
        UserData.ucInputSource = GetInputPortName(OSDindex);
        OSDConfirmInput();
...
void OSDEnterItm11(void)
{
    ...
    OSDEnterAuto();
        if((UserData.bSensEyeDemo == ON) && (isOSDAllowSenseyeDemo()))
    {
        UserData.ucDualViewMode = PICTURE_DUALMODE_OFF;
        UserData.bSensEyeDemo = OFF;
        SetDemo();            
        SetOSDUpdUsrFlag();
    }
...
void OSDEnterItm32(void)
{
    ...
    if(UserData.bSensEyeDemo!=PICTURE_DUALMODE_OFF){
        usOSDState = OSD_ITEM_3_2_1_STATE + UserData.ucDualViewMode;
    }
    else{
        usOSDState = OSD_ITEM_3_2_1_STATE;
    }
    Custom_System.ucDualViewTemp = UserData.ucDualViewMode;
...
{
    UCHAR temp,i;
    if(usOSDState == OSD_ITEM_3_2_1_STATE){
        Custom_System.bShowSensEyeDemoTemp = UserData.bSensEyeDemo;/////here    
        ucLayer3MenuType = CursorPos(OSD_ITEM_3_1_END,usOSDState);
        OSDClrLayer3Menu(CursorPos(OSD_ITEM_3_1_END,usOSDState));
        ...
        usOSDState = OSD_SUBITEM_3_2_1_STATE_BEG + UserData.bSensEyeDemo;
        temp = CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG,usOSDState);
         if(temp >= 16){
            temp -= 16;
         }
         else if(temp >= 8){
        ...
        OSDSelectLayer4Item(CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG, usOSDState),OSD_LAY4_LANG_NONE);
    }
    else{
        if(UserData.ucDualViewMode != PICTURE_DUALMODE_ONOFF){
            Custom_System.ucDualViewTemp = UserData.ucDualViewMode;
        }
        OSDExitItm32();
        SetOSDUpdUsrFlag();    
...
{
#if 0//FORCE_NON_CIRCLE_ACTION == ON
    if (CursorPos(OSD_ITEM_3_2_1_STATE, usOSDState) <= GetMinLayer3ValidOSDState(OSD_ITEM_3_2_1_STATE, OSD_ITEM_3_2_END)){
        return;
    }
#endif // FORCE_NON_CIRCLE_ACTION
    if(UserData.bSensEyeDemo == PICTURE_DUALMODE_OFF){
    ...
    while (1) {
        ...
        if (OSDIsLayer3ItemValid(CursorPos(OSD_ITEM_3_2_1_STATE, usOSDState)) == TRUE)
            break;
    }
    
    if(CursorPos(OSD_ITEM_3_2_1_STATE,usOSDState) != PICTURE_DUALMODE_ONOFF){
        UserData.ucDualViewMode= CursorPos(OSD_ITEM_3_2_1_STATE,usOSDState); 
    }
    OSDSelectLayer3Item(CursorPos(OSD_ITEM_3_2_1_STATE, usOSDState));
    UserSetDualView(UserData.bSensEyeDemo,UserData.ucDualViewMode);
}
...
{
#if 0//FORCE_NON_CIRCLE_ACTION == ON
    if (CursorPos(OSD_ITEM_3_2_1_STATE, usOSDState) >= GetMaxLayer3ValidOSDState(OSD_ITEM_3_2_1_STATE, OSD_ITEM_3_2_END)){
        return;
    }
#endif // FORCE_NON_CIRCLE_ACTION
    if(UserData.bSensEyeDemo == PICTURE_DUALMODE_OFF){
...
void OSDExitSubItm313(void)
{
    ...
    usOSDState = OSD_ITEM_3_1_7_STATE;
...
void OSDExitSubItm321(void){
    ...
    OSDExitSubItms(CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG,usOSDState));
    usOSDState = OSD_ITEM_3_2_1_STATE;
    OSDSelectLayer3Item(CursorPos(OSD_ITEM_3_2_1_STATE, usOSDState));
}

void OSDConfirmItm321(void){
    Custom_System.bShowSensEyeDemoTemp = UserData.bSensEyeDemo;
    OSDExitSubItms(CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG,usOSDState));
    usOSDState = OSD_ITEM_3_2_1_STATE;
    OSDSelectLayer3Item(CursorPos(OSD_ITEM_3_2_1_STATE, usOSDState));
    SetOSDUpdUsrFlag();
...
#if 0//FORCE_NON_CIRCLE_ACTION == ON
        if (CursorPos(OSD_SUBITEM_3_2_1_1_STATE, usOSDState) <= GetMinLayer3ValidOSDState(OSD_SUBITEM_3_2_1_1_STATE, OSD_SUBITEM_3_2_1_END)){
            return;
        }
#endif // FORCE_NON_CIRCLE_ACTION
    
    OSDUnSelectLBLLayer4Item(CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG,usOSDState));
    usOSDState = PrevState(OSD_SUBITEM_3_2_1_STATE_BEG,OSD_SUBITEM_3_2_1_END,usOSDState);
    OSDSelectLBLLayer4Item(CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG, usOSDState));
    OSDSenseyeDemoUp();
}
void OSDSubItm321Down(void){
#if 0//FORCE_NON_CIRCLE_ACTION == ON
            if (CursorPos(OSD_SUBITEM_3_2_1_1_STATE, usOSDState) >= GetMaxLayer3ValidOSDState(OSD_SUBITEM_3_2_1_1_STATE, OSD_SUBITEM_3_2_1_END)){
                return;
            }
#endif // FORCE_NON_CIRCLE_ACTION
    
    OSDUnSelectLBLLayer4Item(CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG, usOSDState));
    usOSDState = NextState(OSD_SUBITEM_3_2_1_STATE_BEG,OSD_SUBITEM_3_2_1_END,usOSDState);
    OSDSelectLBLLayer4Item(CursorPos(OSD_SUBITEM_3_2_1_STATE_BEG, usOSDState));
    OSDSenseyeDemoUp();
}
...
void OSDExitSubItmAnimation(void)
{
    ...
    usOSDState = OSD_ITEM_3_1_5_STATE;
    OSDSelectLayer3Item(CursorPos(OSD_ITEM_2_9_END, usOSDState));
}
...
UCHAR temp;
  ...
  //  usOSDState = OSD_SUBITEM_4_1_1_BEG + LangItemToPoint(UserData.ucLanguage);
...
void OSDExitSubItm411(void)
{
    ...
    for (i=0; i<MAINMENU_ICON_NUM; i++) {
        for(j=0;j<2;j++){
            OSDAPI_WriteXYRepeatChar(FALSE, ucMainIconPos[i*2], ucMainIconPos[i*2+1]+j, 20, _SPACE_);
        }
    }
    for (i=0; i<MAINMENU_ICON_NUM; i++) {
...
#if 1
/*
    item = LangItemToPoint(Custom_System.ucLanguage);
    if (LangPointToItem(CursorPos(OSD_SUBITEM_4_1_1_BEG, usOSDState)) == OSDLANG_NONE) {
        Custom_System.ucLanguage = LangPointToItem(0);
    }
    else {
        Custom_System.ucLanguage = LangPointToItem(item);
    }*/
Custom_System.ucLanguage = LangPointToItem(CursorPos(OSD_SUBITEM_4_1_1_BEG, usOSDState));
#else   
...
/*
    item = LangItemToPoint(Custom_System.ucLanguage);
    if (LangPointToItem(CursorPos(OSD_SUBITEM_4_1_1_BEG, usOSDState)) == OSDLANG_NONE) {
        Custom_System.ucLanguage = LangPointToItem(GetLangTotalNum()-1);
    }
    else {
        Custom_System.ucLanguage = LangPointToItem(item);
    }*/
...
void OSDEnterNoSync(void)
{
	...
	{
		usOSDState = OSD_INFO_BURNING_STATE;
    	OSDShowBurning();
